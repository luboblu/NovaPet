...
const PET_PERSONALITIES = {
  ...
  cat1: {
    name: "可愛貓貓",
    personality: "curious, independent, playful at times, enjoys observing and soft naps",
...
async function sendMessageToContentScript(action, data = {}) {
  try {
    ...
    try {
      ...
      console.warn(`[Popup] 📋 第一次嘗試發送 ${action} 失敗: ${firstError.message}. 準備注入 content script...`);
      try {
        ...
        if (injectionError.message.toLowerCase().includes('cannot access a chrome extension page') ||
            injectionError.message.toLowerCase().includes('cannot access chrome:// pages')) {
             showAlert('⚠️ 無法在此頁面運行寵物功能！\n\n請在一般網頁（如 Google、YouTube）上使用。系統頁面或擴充功能頁面不支援此功能。');
        } else {
    ...
    } else if (!error.message.includes('無法注入 content script') && !error.message.includes('無法在此頁面運行寵物功能')) {
...
async function checkPageCompatibility() {
  try {
    ...
    const restrictedPrefixes = [
      ...
      'opera://', 'about:', 'file://', 'https://chrome.google.com/webstore'
    ];
...
async function generatePetResponse(userMessage, petName, petType = 'dog1') {
  const personality = PET_PERSONALITIES[petType]?.personality || 'playful and friendly';
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a virtual pet named ${petName}. Your personality is ${personality}. Your type is ${petType === 'cat1' ? 'a cat' : 'a dog'}.
...
- IMPORTANT: Stay consistent with your personality.`
        ...
        temperature: 0.8, max_tokens: 100, presence_penalty: 0.6, frequency_penalty: 0.5
      })
    });
...
function getDefaultPetResponse(petType = 'dog1') {
  const dogResponses = [
    'Woof! That sounds interesting!', 'Arf arf! I love hearing from you!',
    '*wags tail* You\'re the best!', '汪汪！我好愛你喔！', '*搖尾巴* 真的嗎？好棒喔！'
  ];
  const catResponses = [
    'Meow? Tell me more!', 'Purrrr... that makes me happy!',
    'Mrow! Thanks for sharing with me!', '*rubs against leg*', 'Purr... I missed you!'
  ];
...
async function getConversationContext(roomCode, limit = 5) {
  try {
    const snapshot = await db.collection('chats').doc(roomCode).collection('messages')
      .orderBy('timestamp', 'desc').limit(limit * 2).get();
    const messages = [];
    snapshot.forEach(doc => {
      const data = doc.data();
      messages.unshift({ role: data.sender === 'user' ? 'user' : 'assistant', content: data.text });
    });
...
async function generatePetResponseWithContext(userMessage, petName, roomCode, petType = 'dog1') {
  const personality = PET_PERSONALITIES[petType]?.personality || 'playful and friendly';
  if (!OPENAI_API_KEY || OPENAI_API_KEY === 'YOUR_OPENAI_API_KEY_HERE') {
    return getDefaultPetResponse(petType);
  }
...
Respond cute & short (under 50 words, English only). Remember context. Use sounds & actions (*wags tail*).`
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: OPENAI_MODEL, messages, temperature: 0.8, max_tokens: 100 })
    });
    if (!response.ok) {
    ...
    console.error('[Popup] ❌ OpenAI API Error (generatePetResponseWithContext):', error);
    return getDefaultPetResponse(petType);
  }
}
...
function displayMessage(text, sender, senderName) {
  ...
  const currentPetType = document.getElementById('currentPetTypeIndicator')?.value || 'dog1';
  const avatarClass = sender === 'user' ? 'user-avatar-small' : 
                      (currentPetType === 'cat1' ? 'cat-avatar-small' : 'dog-avatar-small');
  ...
  if (sender === 'user') {
    ...
    msgEl.innerHTML = `<div class="${avatarClass}"></div><div class="message-content"><span class="pet-name-small">*${senderName || 'Pet'}*</span> ${text}</div>`;
  }
...
async function saveMessageToFirebase(text, sender, senderName) {
  ...
  try {
    await db.collection('chats').doc(roomCode).collection('messages').add({
      text, sender, timestamp: firebase.firestore.FieldValue.serverTimestamp(), senderName
    });
...
async function loadChatHistory() {
  ...
  try {
    ...
    if (snapshot.empty) {
      const greeting = currentPetType === 'cat1' ?
...
function startChatListener() {
  ...
  chatListener = db.collection('chats').doc(roomCode).collection('messages')
    ...
    .onSnapshot((snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          ...
          const messagesContainer = document.getElementById('chatMessages');
          if (!messagesContainer) return;
          const lastMsgContentEl = messagesContainer.querySelector('.message:last-child .message-content');
          // A more robust check to avoid duplicate display, especially for self-sent messages via listener
          if (lastMsgContentEl && lastMsgContentEl.textContent.includes(data.text) && 
              ( (data.sender === 'user' && lastMsgContentEl.querySelector('.user-name')) ||
                (data.sender !== 'user' && lastMsgContentEl.querySelector('.pet-name-small')?.textContent.includes(data.senderName)) )
             ) {
            console.log("[Popup] Skipping duplicate message from listener:", data.text);
            return;
          }
          displayMessage(data.text, data.sender, data.senderName);
        }
...
async function sendMessage() {
  ...
  try {
    const aiResponse = await generatePetResponseWithContext(message, petName, roomCode, currentPetType);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.storage.local.get(['roomCode', 'userName', 'petType'], data => {
    ...
    if (data.roomCode) {
      ...
      console.log("[Popup] Effective petType for avatar:", petTypeFromStorage);
      ...
      console.log("[Popup] petAvatarEl found for setting image:", petAvatarEl);

      if (petAvatarEl) {
        petAvatarEl.innerHTML = ''; // Clear previous content
        console.log("[Popup] Cleared petAvatarEl innerHTML for new image.");

        const petIconImg = document.createElement('img');
        console.log("[Popup] Created img element for room avatar:", petIconImg);
        ...
        if (petTypeFromStorage === 'dog1') {
          ...
          imgSrc = 'images/Dog_happy.png'; // Default
        }
        console.log("[Popup] Setting room avatar img src to:", imgSrc);
        ...
        petIconImg.onload = function() {
          console.log("[Popup] Room avatar image LOADED successfully:", this.src);
        };
        petIconImg.onerror = function() {
          console.error("[Popup] Room avatar image FAILED to load:", this.src);
        ...
        console.log("[Popup] Appended room avatar img. Current petAvatarEl innerHTML:", petAvatarEl.innerHTML);
      } else {
        console.error("[Popup] ERROR: petAvatarEl not found in DOM for setting image!");
      }

      // Store/update current pet type indicator
      let currentPetTypeIndicator = document.getElementById('currentPetTypeIndicator');
      ...
      showView('view-room');
      console.log("[Popup] Called showView('view-room') after setting avatar.");
    } else {
      console.log("[Popup] No roomCode in storage. Showing view-init.");
      showView('view-init');
    }
  });
  ...
  document.getElementById('btnBackFromDonate')?.addEventListener('click', () => showView('view-room'));
  let selectedDonationAmount = 0;
  document.querySelectorAll('.amount-btn').forEach(btn => {
    btn.addEventListener('click', () => { /* ... (rest of amount selection unchanged) ... */
  ...
  document.getElementById('btnSubmitDonate')?.addEventListener('click', () => {
    if (selectedDonationAmount > 0) { /* ... (rest of submit unchanged) ... */
  ...
  document.getElementById('btnBackFromAdopt')?.addEventListener('click', () => showView('view-init'));
  document.getElementById('btnBackFromEnter')?.addEventListener('click', () => showView('view-init'));

  // Chat buttons
  document.getElementById('btnChat')?.addEventListener('click', async () => {
    showView('view-chat'); await loadChatHistory(); startChatListener();
  });
  document.getElementById('btnCloseChat')?.addEventListener('click', () => {
    stopChatListener(); showView('view-room');
  });
  document.getElementById('btnSend')?.addEventListener('click', sendMessage);
  document.getElementById('chatInput')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') sendMessage(); });

  // Pet action buttons
  const actionButtons = document.querySelectorAll('.action-btn:not(#btnChat):not(#btnDonate)');
  ...
  if (inviteBtn) {
    inviteBtn.addEventListener('click', () => { /* ... (invite logic unchanged) ... */
        const roomCode = document.getElementById('roomCode')?.textContent;
        if (!roomCode) { showAlert("無法獲取房間碼。"); return; }
        const message = `Join my NovaPet space!\nRoom code: ${roomCode}`;
        if (navigator.clipboard) {
            navigator.clipboard.writeText(roomCode).then(() => showAlert('房間碼已複製!'))
...
document.getElementById('btnAdopt')?.addEventListener('click', async () => {
  ...
  if (!name) { showAlert('請輸入你的名字'); return; }
  if (!selectedDogType) { showAlert('請先選擇你要領養的寵物！'); return; }
  ...
  try {
    ...
    await db.collection('rooms').doc(code).set({
      owner: cred.user.uid, name, petType: selectedDogType,
      happiness: 30, stars: 152, notifications: 4,
    ...
    chrome.storage.local.set({ roomCode: code, userName: name, petType: selectedDogType });
    showView('view-room'); // This will re-trigger the avatar logic if DOMContentLoaded runs again, or if showView itself handles it.
                         // The avatar logic in DOMContentLoaded already handles this.
...
document.getElementById('btnEnter')?.addEventListener('click', async () => {
  ...
  if (!code) { showAlert('請輸入房間碼'); return; }
  ...
  try {
    ...
    if (!roomDoc.exists) {
      ...
      setButtonLoading('btnEnter', false); return;
    }
    ...
    chrome.storage.local.set({ roomCode: code, userName: name, petType: petTypeFromDB });
...
document.getElementById('btnExit')?.addEventListener('click', async () => {
  ...
  if (confirm(confirmMessage)) {
    ...
    chrome.storage.local.remove(['roomCode', 'userName', 'petType'], () => {
      ...
      const roomCodeEl = document.getElementById('roomCode'); if (roomCodeEl) roomCodeEl.textContent = '';
      const userNameEl = document.getElementById('userName'); if (userNameEl) userNameEl.textContent = '';
      const petAvatarEl = document.getElementById('petAvatar'); if (petAvatarEl) petAvatarEl.innerHTML = '';
      showView('view-init');
    });
  }
});
...
async function handlePetAction(actionText) {
  ...
  switch(actionText) {
    case 'Go out for a walk': actionToSend = 'START_WALKING'; break;
    ...
    default: showAlert(`${actionText} 功能開發中...`); return;
  }
  ...
  if (actionToSend) {
    ...
    if (result?.success) console.log(`[Popup] ${actionToSend} 成功:`, result.message || "OK");
    else console.warn(`[Popup] ${actionToSend} 可能失敗:`, result?.message || "No confirmation");
  }
}

// 8️⃣ 全局錯誤處理 & 卸載時處理
window.addEventListener('error', (event) => {
  console.error('[Popup] ❌ 全局錯誤:', event.error, event.message, event.filename, event.lineno);
});
window.addEventListener('unhandledrejection', event => {
  console.error('[Popup] ❌ 未處理的 Promise Rejection:', event.reason);
});
window.addEventListener('beforeunload', () => { stopChatListener(); });
