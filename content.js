...
// ================ 0.1 預設寵物對話生成 ================
async function generatePetDialogue(personality, petName, action = "") {
  // Renamed from generateDogDialogue
...
function getDefaultDialogue(action) {
  const dialogues = {
    // Generic, or could be split by pet type if needed
    walk: [
      "Woof! Walk time!",
      "Yay! Outside!",
      "Meow! Let's explore!",
...
class PetManager {
  ...
  constructor() {
    ...
    this.dog1MoodImages = [
      "images/Dog_calm.png",
      "images/Dog_confuse.png",
      "images/Dog_excited.png",
      "images/Dog_happy.png",
      "images/Dog_sad.png",
      "images/Dog_sleepy.png",
    ];
    this.cat1MoodImages = [
      "images/Cat_calm.png",
      "images/Cat_confuse.png",
      "images/Cat_excited.png",
      "images/Cat_happy.png",
      "images/Cat_sad.png",
      "images/Cat_sleepy.png",
    ];
  ...
  async initializePet(petNameFromPopup, personalityFromPopup) {
    // Renamed from initializeDog
    ...
    console.log(
      `[Content] Initializing pet. Name: ${this.petData.name}, Type: ${this.petType}, Personality: ${this.petData.personality}`
    );
    ...
    console.log(
      `🐾 ${this.petData.name} (Type: ${this.petType}) 已經出現在頁面上！點擊寵物顯示控制面板`
    );
  }

  async createPetElements() {
    // Renamed
    ...
    if (this.petType === "dog1") {
      initialImgPath =
        this.dog1MoodImages[
          Math.floor(Math.random() * this.dog1MoodImages.length)
        ];
    } else if (this.petType === "cat1") {
      initialImgPath =
        this.cat1MoodImages[
          Math.floor(Math.random() * this.cat1MoodImages.length)
        ];
    } else {
  ...
  updatePetImageRandomly() {
    ...
    if (
      !this.currentPetImageElement ||
      this.currentPetImageElement.tagName !== "IMG"
    ) {
      console.warn(
        "[Content] Cannot update image, current element is not an IMG tag or is null."
      );
      return;
    }
    ...
    if (
      this.currentPetImageElement.src.endsWith(newImgPath) &&
      imageList.length > 1
    ) {
  ...
  createControlPanel() {
    ...
    console.log(
      "[Content] Creating control panel. this.petType is:",
      this.petType
    );
    ...
    this.controlPanel.addEventListener("click", async (e) => {
      ...
      switch (action) {
        ...
        case "treat":
          ...
          break;
        case "pet":
          await this.showPetDialogue("pet");
          this.addHappinessEffect("💝");
          break;
        case "toy":
  ...
  toggleFollowMode() {
    ...
    if (this.isFollowing) {
      ...
      this.stopFollowMode();
      this.startAutoMovement();
      this.showPetDialogue(
        this.petType === "cat1" ? "Hmph, fine." : "我自由了～"
      ); // Cat might be more aloof
    }
  }

  startFollowMode() {
    document.addEventListener("mousemove", this.followMouseHandler);
  }
  stopFollowMode() {
    document.removeEventListener("mousemove", this.followMouseHandler);
  }
  ...
  followMouse(e) {
    ...
    this.petContainer.style.left = `${Math.max(
      0,
      Math.min(window.innerWidth - 100, targetX)
    )}px`;
    this.petContainer.style.top = `${Math.max(
      0,
      Math.min(window.innerHeight - 100, targetY)
    )}px`;
  ...
  setupEventListeners() {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      x = Math.max(
        0,
        Math.min(window.innerWidth - this.petContainer.offsetWidth, x)
      );
      y = Math.max(
        0,
        Math.min(window.innerHeight - this.petContainer.offsetHeight, y)
      );
  ...
  async movePetRandomly() {
    ...
    const x =
      Math.random() * (window.innerWidth - this.petContainer.offsetWidth);
    const y =
      Math.random() * (window.innerHeight - this.petContainer.offsetHeight);
    this.petContainer.style.left = `${x}px`;
    this.petContainer.style.top = `${y}px`;
    this.updateControlPanelPosition("none");
  ...
  async showPetDialogue(action = "") {
    // Renamed
    if (!this.petDialog) return;
    const dialogue = await generatePetDialogue(
      this.petData.personality,
      this.petData.name,
      action
    );
  ...
  async performAction(action, data = {}) {
    ...
    console.log(
      `[Content] Performing action: ${action} for petType: ${this.petType}`
    );
    switch (action) {
  ...
  async parkMode() {
    ...
    for (let i = 0; i < 3; i++) {
      setTimeout(() => {
        if (this.petContainer) this.movePetRandomly();
      }, i * 2000);
    }
    setTimeout(() => {
      if (!this.isFollowing && this.petContainer) this.startAutoMovement();
    }, 6000);
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (action) {
    case "START_WALKING":
      ...
      sendResponse({
        success: true,
        message: `${dogName} 的寵物開始散步！點擊寵物顯示控制面板`,
      });
      break;
...
setInterval(() => {
  chrome.storage.local.get(["roomCode"], (data) => {
    if (!data.roomCode && dogManager.petContainer) {
      // Check petContainer
